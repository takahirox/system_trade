-- generate topix monthly basic data from topix daily
insert into topix_monthly(date, open, high, low, close)
...
-- generate topix monthly id
update topix_monthly t1
  inner join (
    select t.date date,
           @n:=@n+1 id
    from topix_monthly t,
         (select @n:=0) dmy
    order by t.date
  ) t2
  on t1.date=t2.date
...
-- generate topix_monthly candle and beard
update topix_monthly t1
  inner join (
    select date date,
           close-open candle,
