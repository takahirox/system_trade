-- generate nk225 monthly basic data from nk225 daily
insert into nk225_monthly(date, open, high, low, close)
...
-- generate nk225 monthly id
update nk225_monthly t1
  inner join (
    select t.date date,
           @n:=@n+1 id
    from nk225_monthly t,
         (select @n:=0) dmy
    order by t.date
  ) t2
  on t1.date=t2.date
...
-- generate nk225_monthly candle and beard
update nk225_monthly t1
  inner join (
    select date date,
           close-open candle,
