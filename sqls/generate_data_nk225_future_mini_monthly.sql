-- generate nk225_future_mini monthly basic data from nk225_future_mini daily
insert into nk225_future_mini_monthly(date, open, high, low, close)
...
update nk225_future_mini_monthly t1
  inner join (
    select t.date date,
           @n:=@n+1 id
    from nk225_future_mini_monthly t,
         (select @n:=0) dmy
    order by t.date
  ) t2
  on t1.date=t2.date
...
update nk225_future_mini_monthly t3
  inner join (
    select t1.date date,
           t1.close-t2.close change_price,
           (t1.close-t2.close)/t2.close*100 change_percentage
    from nk225_future_mini_monthly t1,
         nk225_future_mini_monthly t2
    where t1.id=t2.id+1
  ) t4
  on t3.date=t4.date
...
-- generate nk225_future_mini_monthly candle and beard
update nk225_future_mini_monthly t1
  inner join (
    select date date,
           close-open candle,
    ...
    from nk225_future_mini_monthly
    order by date
  ) t2
  on t1.date=t2.date
...
-- generate nk225_future_mini_monthly beard
update nk225_future_mini_monthly t1
  inner join (
    select date date,
           close-open candle
