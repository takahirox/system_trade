-- generate usdjpy monthly basic data from usdjpy daily
insert into usdjpy_monthly(date, open, high, low, close)
...
-- generate usdjpy monthly id
update usdjpy_monthly t1
  inner join (
    select t.date date,
           @n:=@n+1 id
    from usdjpy_monthly t,
         (select @n:=0) dmy
    order by t.date
  ) t2
  on t1.date=t2.date
...
-- generate usdjpy_monthly candle and beard
update usdjpy_monthly t1
  inner join (
    select date date,
           close-open candle,
