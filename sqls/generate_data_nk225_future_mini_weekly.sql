-- generate nk225_future_mini weekly basic data from nk225_future_mini daily
insert into nk225_future_mini_weekly(date, open, high, low, close)
select min(t.date) date,
       (select t2.open
        from nk225_future_mini_daily t2
...
update nk225_future_mini_weekly t1
  inner join (
    select t.date date,
           @n:=@n+1 id
    from nk225_future_mini_weekly t,
         (select @n:=0) dmy
    order by t.date
  ) t2
  on t1.date=t2.date
...
update nk225_future_mini_weekly t3
  inner join (
    select t1.date date,
           t1.close-t2.close change_price,
           (t1.close-t2.close)/t2.close*100 change_percentage
    from nk225_future_mini_weekly t1,
         nk225_future_mini_weekly t2
    where t1.id=t2.id+1
  ) t4
  on t3.date=t4.date
...
-- generate nk225_future_mini_weekly candle and beard
update nk225_future_mini_weekly t1
  inner join (
    select date date,
           close-open candle,
...
-- generate nk225_future_mini_weekly beard
update nk225_future_mini_weekly t1
  inner join (
    select date date,
           close-open candle
