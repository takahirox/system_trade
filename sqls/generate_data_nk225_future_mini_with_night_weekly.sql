-- generate nk225_future_mini_with_night weekly basic data
-- from nk225_future_mini_with_night daily
insert into nk225_future_mini_weekly_with_night(date, open, high, low, close)
...
-- generate nk225_future_mini weekly id
update nk225_future_mini_weekly_with_night t1
  inner join (
    select t.date date,
           @n:=@n+1 id
    from nk225_future_mini_weekly_with_night t,
         (select @n:=0) dmy
    order by t.date
  ) t2
  on t1.date=t2.date
...
-- generate nk225_future_mini weekly change
update nk225_future_mini_weekly_with_night t3
  inner join (
    select t1.date date,
           t1.close-t2.close change_price,
           (t1.close-t2.close)/t2.close*100 change_percentage
    from nk225_future_mini_weekly_with_night t1,
         nk225_future_mini_weekly_with_night t2
    where t1.id=t2.id+1
  ) t4
  on t3.date=t4.date
...
-- generate nk225_future_mini_weekly_with_night candle and beard
update nk225_future_mini_weekly_with_night t1
  inner join (
    select date date,
           close-open candle,
    ...
    from nk225_future_mini_weekly_with_night
    order by date
  ) t2
  on t1.date=t2.date
...
-- generate nk225_future_mini_weekly_with_night beard
update nk225_future_mini_weekly_with_night t1
  inner join (
    select date date,
           close-open candle
...
-- generate nk225_future_mini weekly ma
update nk225_future_mini_weekly_with_night t3
  inner join (
    select t1.date date,
           avg(t2.close) ma
